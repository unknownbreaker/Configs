...
(function run() {      // this will auto-run when script is executed
  ...
  for (let key in weeksNeeded) {
    weeksNeeded[key].map((weekNum) => {
      if (todayDate === nthDayOfMonth(moment(), days[key], weekNum)) needsSlack = true;
    });
  }
  ...
  if (needsSlack) {
    ...
    return kme.setvariable("isSlackStandup", { to: "slack" });
  }
  return kme.setvariable("isSlackStandup", { to: "zoom" });
})();
