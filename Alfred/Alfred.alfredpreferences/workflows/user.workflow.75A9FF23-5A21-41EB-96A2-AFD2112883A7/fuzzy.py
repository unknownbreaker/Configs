...
class Fuzzy(object):
    ...
    """
	def __init__(self, adj_bonus = adj_bonus, sep_bonus = sep_bonus, camel_bonus = camel_bonus, lead_penalty = lead_penalty, max_lead_penalty = max_lead_penalty, unmatched_penalty = unmatched_penalty, separators = separators):
...
class Cache(object):
	...
	def cache_path(self):
		...
		if not self._cache_path:
			...
			self._cache_path = os.path.join(self.cache_dir, self.session_id + '.json')
...
def main():
	...
	log('cmd=%r, query=%r, session_id=%r', cmd, query, os.getenv(SID))
